...
def return_data(fold,type,scale,data_str):
    ...
    cat_indices = [list(train_input.columns).index('weather'),list(train_input.columns).index('wind_direction')]
    # print(list(train_input.columns))
    ...
    if(scale==True):
        ...
        # train_output = scaler_y.transform(train_output.reshape(-1, 1))[:, 0]
    ...
    return torch.Tensor(train_input).to(device),torch.Tensor(train_output).to(device),torch.Tensor(test_input).to(device), test_output.values, cat_indices, DELTA
